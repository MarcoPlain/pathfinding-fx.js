  ...
  class PathfindingFX {
    constructor(element, settings = {}) {
      ...
      if (!(element instanceof Node)) {
        throw "Can't initialize pathfinding-fx because " + element + " is not a Node.";
      }
      this.nodesList = [];
      ...
      this.lastFrameTimeMs = null, this.maxFPS = 60, this.delta = 0, this.timestep = 1000 / 24;
      ...
      while (open.length) {
        ...
        for (let n = 0; n < neighbors.length; n++) {
          ...
          for (let c = 0; c < closed.length; c++) {
            if (closed[c].pos.x == neighbor.pos.x && closed[c].pos.y == neighbor.pos.y) invalid = true;
          }
          if (invalid) continue;
          let g = currentNode.g;
          // Diagonal g
          if (neighbor.pos.x != currentNode.pos.x && neighbor.pos.y != currentNode.pos.y) {
          ...
          let gBest = false;
          let found = false;
          for (let o = 0; o < open.length; o++) if (open[o].pos.x == neighbor.pos.x && open[o].pos.y == neighbor.pos.y) found = true;
          if (!found) {
            gBest = true;
    ...
    positions = from => {
      ...
      while (open.length) {
        ...
        for (let n = 0; n < neighbors.length; n++) {
          let neighbor = neighbors[n];
          if (neighbor.pos.x == currentNode.pos.x - 1 && neighbor.pos.y == currentNode.pos.y) {
            neighbor.dir = "left";
          }
          if (neighbor.pos.x == currentNode.pos.x + 1 && neighbor.pos.y == currentNode.pos.y) {
            neighbor.dir = "right";
          }
          if (neighbor.pos.x == currentNode.pos.x && neighbor.pos.y == currentNode.pos.y - 1) {
            neighbor.dir = "top";
          }
          if (neighbor.pos.x == currentNode.pos.x && neighbor.pos.y == currentNode.pos.y + 1) {
            neighbor.dir = "bottom";
          }
          neighbor.linear = typeof currentNode.dir == "undefined" || currentNode.linear && currentNode.dir == neighbor.dir;
          let invalid = false;
          for (let c = 0; c < closed.length; c++) {
            if (closed[c].pos.x == neighbor.pos.x && closed[c].pos.y == neighbor.pos.y) invalid = true;
          }
    ...
    neighbors = node => {
      let neighbors = [];
      if (this.matrix[node.pos.y - 1] && this.matrix[node.pos.y - 1][node.pos.x] && this.matrix[node.pos.y - 1][node.pos.x].w) neighbors.push(this.matrix[node.pos.y - 1][node.pos.x]);
      if (this.matrix[node.pos.y + 1] && this.matrix[node.pos.y + 1][node.pos.x] && this.matrix[node.pos.y + 1][node.pos.x].w) neighbors.push(this.matrix[node.pos.y + 1][node.pos.x]);
      if (this.matrix[node.pos.y] && this.matrix[node.pos.y][node.pos.x - 1] && this.matrix[node.pos.y][node.pos.x - 1].w) neighbors.push(this.matrix[node.pos.y][node.pos.x - 1]);
      if (this.matrix[node.pos.y] && this.matrix[node.pos.y][node.pos.x + 1] && this.matrix[node.pos.y][node.pos.x + 1].w) neighbors.push(this.matrix[node.pos.y][node.pos.x + 1]);

      // Diagonal neighbors
      if (this.matrix[node.pos.y - 1] && this.matrix[node.pos.y - 1][node.pos.x - 1] && this.matrix[node.pos.y - 1][node.pos.x - 1].w && this.matrix[node.pos.y - 1][node.pos.x] && this.matrix[node.pos.y - 1][node.pos.x].w && this.matrix[node.pos.y][node.pos.x - 1] && this.matrix[node.pos.y][node.pos.x - 1].w) neighbors.push(this.matrix[node.pos.y - 1][node.pos.x - 1]);
      if (this.matrix[node.pos.y + 1] && this.matrix[node.pos.y + 1][node.pos.x - 1] && this.matrix[node.pos.y + 1][node.pos.x - 1].w && this.matrix[node.pos.y + 1][node.pos.x] && this.matrix[node.pos.y + 1][node.pos.x].w && this.matrix[node.pos.y][node.pos.x - 1] && this.matrix[node.pos.y][node.pos.x - 1].w) neighbors.push(this.matrix[node.pos.y + 1][node.pos.x - 1]);
      if (this.matrix[node.pos.y - 1] && this.matrix[node.pos.y - 1][node.pos.x + 1] && this.matrix[node.pos.y - 1][node.pos.x + 1].w && this.matrix[node.pos.y - 1][node.pos.x] && this.matrix[node.pos.y - 1][node.pos.x].w && this.matrix[node.pos.y][node.pos.x + 1] && this.matrix[node.pos.y][node.pos.x + 1].w) neighbors.push(this.matrix[node.pos.y - 1][node.pos.x + 1]);
      if (this.matrix[node.pos.y + 1] && this.matrix[node.pos.y + 1][node.pos.x + 1] && this.matrix[node.pos.y + 1][node.pos.x + 1].w && this.matrix[node.pos.y + 1][node.pos.x] && this.matrix[node.pos.y + 1][node.pos.x].w && this.matrix[node.pos.y][node.pos.x + 1] && this.matrix[node.pos.y][node.pos.x + 1].w) neighbors.push(this.matrix[node.pos.y + 1][node.pos.x + 1]);
      return neighbors;
    ...
    free = pos => {
      return this.nodesList.findIndex(n => n.pos.x == pos.x && n.pos.y == pos.y) == -1 /*&& this.blockedPositions.findIndex((n) => n.pos.x == pos.x && n.pos.y == pos.y) ==
                                                                                       -1*/;
    };
    ...
    update(delta) {
      ...
      .forEach(node => {
        ...
        if (node.path.length > 1) {
          ...
          node.x += sX;
          node.y += sY;
          var distanceX = Math.abs(node.path[1].pos.x * this.tileSize.w - node.x);
          var distanceY = Math.abs(node.path[1].pos.y * this.tileSize.h - node.y);
    ...
    play() {
      ...
      this.animationFrameId = requestAnimationFrame(this.animation.bind(0, this));
      return this;
    }
    stop() {
    ...
    addNode(node) {
      ...
      node.jump = steps => {
        if (node.path) {
          ...
          node.x = node.pos.x * this.tileSize.w;
          node.y = node.pos.y * this.tileSize.h;
          if (typeof node.onPosChange === "function") node.onPosChange(node, node.pos);

          // When mouse is inside canvas we need to check, if by this change the mouse is now hovering over the node
          if (this.pixelPosition) this.nodeIsHovered(node, this.getXYFromPoint(this.pixelPosition));
          node.findPath();
      ...
      node.findPath = (to = null) => {
        ...
        const isTrapped = typeof this.map[node.pos.y - 1] != "undefined" && typeof this.map[node.pos.y - 1][node.pos.x] != "undefined" && this.map[node.pos.y - 1][node.pos.x] == 0 && typeof this.map[node.pos.y] != "undefined" && typeof this.map[node.pos.y][node.pos.x + 1] != "undefined" && this.map[node.pos.y][node.pos.x + 1] == 0 && typeof this.map[node.pos.y + 1] != "undefined" && typeof this.map[node.pos.y + 1][node.pos.x] != "undefined" && this.map[node.pos.y + 1][node.pos.x] == 0 && typeof this.map[node.pos.y] != "undefined" && typeof this.map[node.pos.y][node.pos.x - 1] != "undefined" && this.map[node.pos.y][node.pos.x - 1] == 0;
    ...
    drawMap() {
      ...
      for (let y = 0; y < this.map.length; y++) {
        for (let x = 0; x < this.map[y].length; x++) {
          const color = this.matrix && this.matrix[y] && this.matrix[y][x] && this.matrix[y][x].style && this.matrix[y][x].style.color ? this.matrix[y][x].style.color : this.map[y][x] === 1 ? this.settings.emptyNodeColor : this.settings.wallNodeColor;
          const edgeColor = this.matrix && this.matrix[y] && this.matrix[y][x] && this.matrix[y][x].style && this.matrix[y][x].style.edgeColor ? this.matrix[y][x].style.edgeColor : this.settings.wallEdgeColor;
          this.drawNode({
            pos: {
              x: x,
              y: y
            ...
            color: color,
            highlightEdges: {
              top: y == 0 && this.map[y][x] != 0 || this.map[y][x] == 0 && this.map[y - 1] && this.map[y - 1][x] != 0 ? {
                color: edgeColor
              } : null,
              right: x == this.map[y].length - 1 && this.map[y][x] != 0 || this.map[y][x] == 0 && typeof this.map[y][x + 1] != "undefined" && this.map[y][x + 1] != 0 ? {
                color: edgeColor
              } : null,
              bottom: y == this.map.length - 1 && this.map[y][x] != 0 || this.map[y][x] == 0 && this.map[y + 1] && this.map[y + 1][x] != 0 ? {
                color: edgeColor
              } : null,
              left: x == 0 && this.map[y][x] != 0 || this.map[y][x] == 0 && typeof this.map[y][x - 1] != "undefined" && this.map[y][x - 1] != 0 ? {
                color: edgeColor
              } : null,
              // also corners
              topLeft: this.map[y][x] == 0 && this.map[y - 1] && this.map[y - 1][x] == 0 && this.map[y][x - 1] == 0 && this.map[y - 1][x - 1] != 0 ? {
                color: edgeColor
              } : null,
              topRight: this.map[y][x] == 0 && this.map[y - 1] && this.map[y - 1][x] == 0 && this.map[y][x + 1] == 0 && this.map[y - 1][x + 1] != 0 ? {
                color: edgeColor
              } : null,
              bottomRight: this.map[y][x] == 0 && this.map[y + 1] && this.map[y + 1][x] == 0 && this.map[y][x + 1] == 0 && this.map[y + 1][x + 1] != 0 ? {
                color: edgeColor
              } : null,
              bottomLeft: this.map[y][x] == 0 && this.map[y + 1] && this.map[y + 1][x] == 0 && this.map[y][x - 1] == 0 && this.map[y + 1][x - 1] != 0 ? {
                color: edgeColor
              } : null
            }
          });
    ...
    drawNode(node, config = {}) {
      ...
      var drawX = (node.x || node.pos.x * this.tileSize.w) + (typeof node.style != "undefined" && typeof node.style.size != "undefined" ? (this.tileSize.w - this.tileSize.w * node.style.size.w) / 2 : 0);
      var drawY = (node.y || node.pos.y * this.tileSize.h) + (typeof node.style != "undefined" && typeof node.style.size != "undefined" ? (this.tileSize.h - this.tileSize.h * node.style.size.h) / 2 : 0);
      var sizeX = typeof node.style != "undefined" && typeof node.style.size != "undefined" ? node.style.size.w * this.tileSize.w : this.tileSize.w;
      var sizeY = typeof node.style != "undefined" && typeof node.style.size != "undefined" ? node.style.size.h * this.tileSize.h : this.tileSize.h;
      this.ctx.fillStyle = typeof node.style != "undefined" ? node.style.color : config.color || this.settings.pathNodeColor;
      this.ctx.strokeStyle = typeof node.style != "undefined" ? node.style.color : config.color || this.settings.pathNodeColor;
      switch (mode) {
        case "fill":
          switch (shape) {
            case "rect":
            ...
            case "circle":
              this.ctx.beginPath();
              this.ctx.arc(drawX + sizeX / 2, drawY + sizeY / 2, (sizeX + sizeY) / 2 / 2,
              // First normalize w and h and the divide by 2
              0, 2 * Math.PI);
              this.ctx.fill();
              this.ctx.stroke();
              break;
        ...
        case "node":
          setPos = pos => {
            if (typeof this.onInteractionWithANode === "function") {
              ...
              // Determine if node can have new position
              if (this.map[pos.y] && this.map[pos.y][pos.x] && (node.pos.x != pos.x || node.pos.y != pos.y)) {
                node.pos = pos;
                node._positions = null;
                ...
                if (typeof node.onPosChange === "function") node.onPosChange(node, pos);
    ...
    detectContext(pos, event) {
      ...
      let nodeIndex = this.nodesList.filter(n => n.interactive).findIndex(n => this.nodeIsHovered(n, pos, this.normalizePointFromEvent(event)));
      if (nodeIndex >= 0) {
        return {
      ...
      nodeIndex = this.nodesList.filter(n => n.to).map(n => n.to).findIndex(n => this.nodeIsHovered(n, pos, this.normalizePointFromEvent(event)));
      if (nodeIndex >= 0) {
        return {
      ...
      if (typeof this.map[pos.y] == "undefined" || typeof this.map[pos.y][pos.x] == "undefined") {
        return false;
      }
    ...
    mouseMove(event) {
      ...
      this.position = this.getXYFromPoint(this.pixelPosition);
      if (this.interactionFocus) {
        if (this.interactionFocus.pos.x !== this.position.x || this.interactionFocus.pos.y !== this.position.y) {
          this.interactionFocus.setPos(this.position);
        }
      } else {
        this.detectContext(this.position, event);
