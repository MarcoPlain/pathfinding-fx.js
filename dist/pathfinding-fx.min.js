var PathfindingFX = function () {
  ...
  class PathfindingFX {
    constructor(element, map = null, settings = {}) {
    ...
    _find = (from, to = null, _params = {}) => {
      ...
      while (open.length) {
        ...
        for (let n = 0; n < _neighbors.length; n++) {
          ...
          if (!found) {
            gBest = true;
            neighbor.h = to !== null ? Math.round(this._distance(neighbor.pos, to.pos)) : 0;
            open.push(neighbor);
     ...
     */
    free = pos => {
      return this.nodesList.findIndex(n => n.pos.x == pos.x && n.pos.y == pos.y) == -1 && this.nodesList.filter(n => n.to).map(n => n.to).findIndex(n => n.pos.x == pos.x && n.pos.y == pos.y) == -1;
    };
    ...
    _update(delta) {
      ...
      .forEach(node => {
        ...
        if (node.path.length > 1) {
          ...
          node.x += sX;
          node.y += sY;
          var _distanceX = Math.abs(node.path[1].pos.x * this.tileSize.w - node.x);
          var _distanceY = Math.abs(node.path[1].pos.y * this.tileSize.h - node.y);
        ...
        case "free":
          setPos = pos => {
            if (typeof this.onInteractionWithAFreeNode === "function") {
              ...
              if (this.settings.interactive && this.free(pos) && typeof this.map[pos.y] != "undefined" && typeof this.map[pos.y][pos.x] != "undefined") {
